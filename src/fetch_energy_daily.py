...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fetch SolarEdge daily energy data.")
    ...
    if args.chunk_days < 1:
        print("chunk-days must be >= 1", file=sys.stderr)
        return 1

    output_path = Path(
        args.output
        or (
            "output/energy_daily_"
            f"{args.site_id}_{start_date.isoformat()}_{end_date.isoformat()}.csv"
        )
    )
